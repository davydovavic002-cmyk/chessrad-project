$(document).ready(function() {
    ...
    socket.on('gameOver', (data) => {
        ...
        switch (data.type) {
            case 'checkmate': statusMessage = `Мат! Победил(а) ${winnerUsername}.`; break;
            case 'resign': statusMessage = `Соперник сдался. Победил(а) ${winnerUsername}.`; break;
            case 'draw': statusMessage = 'Ничья.'; break;
            case 'stalemate': statusMessage = 'Ничья (пат).'; break;
            default: statusMessage = `Ничья (${data.reason || 'неизвестная причина'}).`;
        }
